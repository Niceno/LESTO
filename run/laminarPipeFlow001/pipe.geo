...
D   = 4.8e-3;  // diameter of the pipe
Rad = D/2;     // radius of the pipe

L   = 1.06;    // length of the pipe
// it will be considered only the portion of interest of the pipe
L1  = 0.00;    // (sample is placed at 0.04 m)
L2  = 0.69;    // nothing happens after this point
...
//  Options
side           =  6;  // number of sides in the core (must be a multiple of 4 to avoid issues with ellipse generation)
...
N_CORE                 =   7;       // number of nodes in the radial direction inside the core (0.20 mm)
N_MIDDLE               =   9;       // number of nodes in the radial direction inside the middle layer (~0.12 mm)
N_BLAYER               =   5;       // number of nodes in the radial direction inside the boundary layer (~0.07 mm)

N_STREAM               = L*400 + 1; // number of nodes in the inlet stream-wise direction (2.5 mm)
N_ARC                  = N_CORE;    // number of nodes in the arches and ellipses (0.20 mm, 0.26 mm, 0.42 mm) 
...
pc++;  Point(pc) = {L1, 0, 0 };  // inlet center
pc++;  Point(pc) = {L2, 0, 0 };  // outlet center

// Points in the core 
core_pnt_start = pc + 1;
Printf("Creating core points from point %g", core_pnt_start);
from = 1;  to = side;
x = L1;  r = R(1);  Call NPointsAroundX; 
x = L2;  r = R(1);  Call NPointsAroundX; 

// Points in the boundary layer
middle_pnt_start = pc + 1; 
Printf("Creating middlet points from point %g", middle_pnt_start);
from = 1;  to = side;
x = L1;  r = R(2);  Call NPointsAroundX;
x = L2;  r = R(2);  Call NPointsAroundX;
...
from = 1;  to = side;
x = L1;  r = R(3);  Call NPointsAroundX;
x = L2;  r = R(3);  Call NPointsAroundX;
...
Physical Surface("WALL") = { blayerTan_sur_start : blayerAxi_sur_start - 1 };
